...
fn main() {
    ...
    let mut vec: Vec<u8> = Vec::from([
        ...
        0x37, 0x38, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x51, 0x52, 0x53, 0x54, 0x55,
        0x56, 0x57, 0x58, 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x71, 0x72, 0x73, 0x74,
        0x75, 0x76, 0x77, 0x78,
    ]);
    ...
    let testage_raw = testage.raw();
    println!("testage: {:02x?}", &testage_raw);
    println!("testage: {} bytes", &testage_raw.len());

    println!("messages equal: {}", message == testage);
    println!("decoded payload: {:02x?}", testage.decode());
}
